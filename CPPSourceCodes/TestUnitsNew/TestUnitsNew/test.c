...
#define __INIT_test_IMP
#include "test.h"

ULONG WINAPI
ControlCallback_test_1(
...
)
{
    ...
    Status = PerfStartProvider(
            (LPGUID) & ProviderGuid_test_1,
            (PERFLIBREQUEST) ControlCallback_test_1,
            & hDataSource_test_1);
    ...
    Status = PerfSetCounterSetInfo(
            hDataSource_test_1,
            (PPERF_COUNTERSET_INFO) & CtrSet_test_1_1,
            dwCtrSet_test_1_1);
    if (Status != ERROR_SUCCESS) {
        goto Cleanup;
    }
    ...
    Status = PerfSetCounterSetInfo(
            hDataSource_test_1,
            (PPERF_COUNTERSET_INFO) & CtrSet_test_1_2,
            dwCtrSet_test_1_2);
    if (Status != ERROR_SUCCESS) {
        goto Cleanup;
    }
...
Cleanup:
    if (Status != ERROR_SUCCESS) {
        PerfStopProvider(hDataSource_test_1);
    }
    return Status;
}
...
)
{
    ...
    Status = PerfStopProvider(hDataSource_test_1);
